...
void PenWidget::paintEvent(QPaintEvent *event)
{
    ...
    Proj& proj = mEd.proj;
    int tw = proj.tileset.tw;
    int th = proj.tileset.th;
    QImage img(tw,th, QImage::Format_RGBX8888);
    RenderCell(img, QPoint(0, 0), proj.tileset, proj.palette, mEd.leftPen);

    QRect out(4, 4, tw*mZoom, th*mZoom);
    painter.drawImage(out, img);
    RenderCell(img, QPoint(0, 0), proj.tileset, proj.palette, mEd.rightPen);
